...
2. ./configure; make; make install でインストールできます。
   ...
   * --with-cabocha でCaboChaのインストール場所（include/cabocha.h と lib/libcabocha.so の親ディレクトリ）を指定できます。通常は不要です。
...
* --possetで、-t=0のときに解析対象となる品詞を指定することができます。詳しくは後述します。
...
--possetオプションでこれを変更することができます。例えば、「ウナギだ」といった名詞＋助動詞を解析対象としたい時は、以下のように--possetオプションを指定します。 
